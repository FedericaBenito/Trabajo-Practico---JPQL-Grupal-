...
public class ClienteManager {
    ...
    public List<Cliente> getTodosLosClientes() {
        String jpql = "FROM Cliente ORDER BY razonSocial ASC";
        Query query = em.createQuery(jpql);
        List<Cliente> clientes = query.getResultList();
        return clientes;
    }
    ...
    public List<Cliente> getClientesConFacturas() {
        String jpql = "SELECT c FROM Cliente c WHERE EXISTS (SELECT 1 FROM Factura f WHERE f.cliente = c)";
        Query query = em.createQuery(jpql);
    ...
    //EXISTS se utiliza para verificar la existencia de registros relacionados.
    //La subconsulta SELECT 1 FROM Factura f WHERE f.cliente = c verifica la existencia de facturas para cada cliente.
